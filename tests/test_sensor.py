...
class TestGenerateDailyStatistics:
    def test_empty_lines(self) -> None:
        assert _generate_daily_statistics([]) == []

    def test_single_line(self) -> None:
        stats = _generate_daily_statistics([_make_line(100.0, 1000.0, "1-January")])
        assert len(stats) == 1
    ...
    def test_multiple_lines(self) -> None:
        stats = _generate_daily_statistics(
            [
        ...
        assert len(stats) == 3
        assert stats[0]["start"] == datetime(YEAR, 1, 1, 0, 0, tzinfo=LONDON_TZ)
        assert stats[1]["start"] == datetime(YEAR, 1, 2, 0, 0, tzinfo=LONDON_TZ)
        assert stats[2]["start"] == datetime(YEAR, 1, 3, 0, 0, tzinfo=LONDON_TZ)
        assert stats[0]["state"] == 100
    ...
    def test_usage_values_are_truncated_to_int(self) -> None:
        stats = _generate_daily_statistics([_make_line(99.7, 1000.3, "1-January")])
